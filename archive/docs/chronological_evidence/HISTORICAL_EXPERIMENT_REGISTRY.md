...
Status: historical chronological registry after the 2026-06-17 stratified split reset.

The metrics below are preserved for traceability, but they are no longer active
thesis-facing results. New experiments must be rerun under
`split_strategy=stratified_holdout` and recorded separately.

## Active Stratified Registry

No stratified rerun has been completed yet.

| ID | Experiment | Split | Status |
|----|------------|-------|--------|
| S0 | Split validation | `stratified_holdout` | Pending |
...
| A1-T | Alharbi-style preprocessing tuned | `stratified_holdout` | Pending |
| A1-AE | AE-LightGBM after strongest A1 baseline | `stratified_holdout` | Blocked until A1-T |

Use `docs/STRATIFIED_SPLIT_RESET.md` and
`docs/PAPER_ANCHORED_PREPROCESSING_RESET.md` for the active rerun plan.

## Historical Chronological Registry

This historical section tracks the original proposal-scope experiments (P01-P04)
and post-diagnostic chronological candidates. Parked work is documented in
...
Interpretation: under the previous chronological protocol, AE-05 was the first post-cleanup AE candidate that beat the tuned raw-feature LightGBM baseline on test PR-AUC with a positive bootstrap interval. After the stratified reset, this remains historical evidence only and must be rerun before becoming an active thesis conclusion.
