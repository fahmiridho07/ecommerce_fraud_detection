"""Train FE-LightGBM with robust LD128 AE reconstruction error appended."""
...
B_MIN_VALIDATION_DELTA_FOR_C = 0.005
...
def add_reconstruction_error_feature(
    X: pd.DataFrame,
...
def validate_feature_alignment(
    ...
    if latent_columns:
        raise ValueError(
            "Latent AE features are not allowed in Experiment B: "
...
def stopping_decision(validation_pr_auc: float) -> dict[str, object]:
    validation_delta = float(validation_pr_auc - DEFAULT_FE_VALIDATION_PR_AUC_REFERENCE)
    return {
        "default_fe_validation_pr_auc_reference": DEFAULT_FE_VALIDATION_PR_AUC_REFERENCE,
        "min_validation_delta_for_c": B_MIN_VALIDATION_DELTA_FOR_C,
        "validation_pr_auc_delta_vs_default_fe": validation_delta,
        "run_c_next": bool(validation_delta >= B_MIN_VALIDATION_DELTA_FOR_C),
        "rule": (
            "Run C if Experiment B beats default FE-LGBM validation PR-AUC by "
            "at least 0.005, unless Experiment A already triggered C."
        ),
    }
...
def run_experiment(output_dir: Path, overwrite: bool) -> dict[str, object]:
    ...
    log("Appending raw AE reconstruction error to FE matrices.")
    X_train = add_reconstruction_error_feature(
        X_train_fe,
        reconstruction_errors["train"],
    )
    X_valid = add_reconstruction_error_feature(
        X_valid_fe,
        reconstruction_errors["validation"],
    )
    X_test = add_reconstruction_error_feature(
        X_test_fe,
    ...
    log("Saving FE reconstruction-error LightGBM outputs.")
    save_json(
    ...
    feature_set_summary = {
        "experiment_type": "fe_lgbm_plus_ae_reconstruction_mse_default",
    ...
    run_config = {
        "phase": "fe_ae_controlled_B_fe_reconstruction_error_default",
        ...
        "leakage_prevention": {
            ...
            "reconstruction_error_source": (
                "Loaded from saved train-fitted robust LD128 AE outputs; no AE "
                "fitting is performed here."
            ),
    ...
    print()
    print("FE + AE Reconstruction Error LightGBM Summary")
    print("=============================================")
    ...
    print(f"Run C next              : {decision['run_c_next']}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train default FE-LGBM with raw robust LD128 AE reconstruction MSE."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=FE_RECON_ERROR_LGBM_OUTPUT_DIR,
        help="Output directory for Experiment B.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Allow replacing a non-empty Experiment B output directory.",
    )
