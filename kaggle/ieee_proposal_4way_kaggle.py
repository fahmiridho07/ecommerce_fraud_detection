...
def build_ae(dim, latent):
    """Autoencoder simetris meniru Du et al. (2023): aktivasi Mish, regularisasi L2
    di tiap lapis, struktur melebar-lalu-menyempit (256->128->latent->128->256)."""
    act = "mish"  # Du et al. memakai Mish (Misra 2019); fallback ke relu bila tak tersedia
    try:
        keras.layers.Dense(1, activation="mish")
    except Exception:
        act = "relu"
    reg = keras.regularizers.l2(1e-5)  # L2 di setiap lapis (Du et al.)
    def dense(units, a=act):
        return keras.layers.Dense(units, activation=a, kernel_regularizer=reg)
    inp = keras.Input(shape=(dim,))
    x = dense(256)(inp)
    x = dense(128)(x)
    z = dense(latent)(x)            # ruang laten (undercomplete)
    x = dense(128)(z)
    x = dense(256)(x)
    out = dense(dim, a="linear")(x)  # rekonstruksi (linear utk fitur ter-standarisasi)
